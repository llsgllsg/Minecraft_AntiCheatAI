...
# 导出 ONNX 模型，并修改 IR 版本为 9
dummy_input = torch.randn(1, 12, 128).to(device)
torch.onnx.export(model, dummy_input, "scaffold_detector_temp.onnx",
                  input_names=['behavior_sequence'], output_names=['output'],
                  dynamic_axes={'behavior_sequence': {0: 'batch_size'}},
                  opset_version=12)

# 加载并修改 IR 版本
model_onnx = onnx.load("scaffold_detector_temp.onnx")
model_onnx.ir_version = 9
onnx.save(model_onnx, "scaffold_detector.onnx")
print("ONNX 模型已保存为 scaffold_detector.onnx (IR version 9)")
